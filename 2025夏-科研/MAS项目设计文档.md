...
okk 由于unity cloud免费项目只有3个permission编辑seat
...
要求2 再与1.0做对比报告 【本地也要一个NLP分析脚本】节后第一天反馈量化对比数据。
【公司数据问题：1 回环收听 希望去除店内把模型回答作为顾客音频进行输入的数据】
【2 较多对话记录的期望回复值缺失 希望添加】
...
模型训练对比汇报。含新老模型首字延迟对比和正式回答评分对比。
备注：以前两次收到的共计2815条单轮对话进行测试。
新模型按照ws协议调用 首token延迟'latency'平均值=1.9217s。
目前部署的老模型首token延迟'old_latency'平均值=2.8141s。
回答评分好/中/差占比见饼图。
需要注意：回答评分完全按照NLP的文本相似度排序 真实的语义仍然需要人工检查。也就是说低分回答不一定语义错误，而高分回答过多也可能存在过拟合。

数据提升需求
1 较多对话记录的期望回复值缺失 希望后续数据能够完整添加。
2 回环收听采集问题 希望去除店内收音时 把模型回答作为顾客提问的数据
3 顾客提问值希望完整 不能连续追加成文本作为多条数据 比如：
【禁止。
禁止未成。
禁止未成年人。
禁止未成年人更。
...
1022 实验室
先运行桌面CV项目 生成一批例子 等老师反馈。okk

再考虑diffusion sdxl管线【服务器或本地 早点来安装和运行】，晚上提供给会议同学。
**直接考虑了comfyUI和字节跳动的USO模型和适配的管线做风格迁移 注意模型是flux okk** 
...
【毛茸茸 好处 好制作拼接 有特点 有卖点】而且脸部就用夸张的毛茸茸的主创团队的图  发给美术同学


也可以开放通道 欢迎大家提交profile和风格化照片。

还有个问题：整个图风格化后 地图覆盖相对比较简单。 分辨率提高的问题可以超分。
【方案可以试试扭曲原始地图 或者在像素整地图上贴上风格化物体】


okk 分享工作流


todo 再给一版全毛茸茸的供选择 okk
...
todo 公司需求 31号！两份新数据训练
训练。[4090服务器的网络问题 ：通过本地反向ssh隧道 不经过服务器等mihomo等代理软件解决 见文件夹的page文件]

okk 开会讨论前端风格问题
场景工作流结论：
30号的底线：至少校园场景要全盘羊毛化。老鼠场景用纸盒子！
【一/场景的工作流建立：先生成【1水体+2草地+3道路的羊毛底图  进行切割和覆盖（不要做羊毛瓦片）就按照目前的像素边缘填入即可】
注意 特殊建筑（含桶楼 体育馆 操场等） 还有多种树木 分别扣出来进行羊毛化 再放入场景。
道具需求 1 每个建筑旁边需要一个 杆子比人高的告示牌 半立体的样子 【用于告知具体建筑名称】
2 具体的道具要分别羊毛化并按照建筑功能放入具体场景 并建立碰撞体。

【二/人物方面：使用夸张卡通画 +【卡通配饰的单元】。再统一羊毛化 生成人物】
注意老鼠场景里 只有老鼠本身要毛茸茸 六种重画  但是场景依然是逼真的纸盒子（演变的纸盒子）

周末工作的结论：
1 采用兼顾像素和编制风格的单元去铺场景！
2 按照工作流做
...
【批量生成羊毛建筑 ai生成完再交给媛媛抠图+增强 放入unity 即可】 okk
...
todo  等待unity场景文件。提交一版场景。
【明早先用unity的psd导入psd 再用tool直接绘制！发挥创意】 okk
【接收媛媛的高清建筑图】 okk
...
okk 【前端要解决ws协议的问题然后联调！！ 目前的方案是前端直接读文件！最简单 且利于重播】
okk 气泡框的编制材质 交给脚本同学

预估11.1开始再做两周。发版：羊毛人物需求：肉身可活动四肢 +  10套 衣裤一整套 + 10个风格化头（表情也可变） + 10种头发 +10种明显的头上配饰（帽子等） 
...
unity可以在版本控制中打开分支图标的unity vcs, 右键switch就可以切换分支。
...
校园场景增加羊毛状文字和logo和校徽。在左侧。okk
需要一个常开的大门素材【yh负责】 okk
建筑上 墙上或转角贴牌标识  okk


todo cs脚本性能优化 减少每帧强制刷新 直接读文件
todo 校园道具交付【yy负责】白天摆放 ing... 需要调整
指标的统计ui（这个最底线的要求是有数字显示 tick 存活 死亡 年龄结构等）【need】

最终 校园场景
初版前端交付演示（备用录制方案 大镜头看规模 小场景看事件）
...
【todo】性能优化   应该通过缩点  已经okk 【角色生成逻辑可以继续优化】 差值优化移动时的抖动逻辑
...
unity分支的合并：plasticSCM 先切换到主分支最新节点 然后右键要合并的其他分支最新点 选择最新节点的【Merge from this changeset】 
【此时如果提示You have changes pending that need to be checked in into yourworkspace.说明主分支还有没提交的变化 先提交到主分支再合并】
解决冲突后就会更新main为最新 记得merge结束后在main提交一下即可。
总结就是主分支先提交干净 切到主 merge新 再提交一次主就行。
...
1115
加班
按照要求md修改技术报告（注意维度描述的统一性）okk

----------

1117

【okk】周末需求 修改一下技术报告对比部分
【okk】ppt一些绘图需求 周末做就行

基本需求已经满足 周一 开会即可。
【okk】unity校园场景slogan生成一下【校园场景搭建一下 powered by】


【只有校园需要】unity打包成webGL 评估硬件性能； 【不需要进度条】
https://chatgpt.com/share/69114c4d-b5dc-800c-a577-566229ad76bb


备注：布置的需求
```
unity打包成webGL；改成socket协议分发 用户浏览器接受log并播放
标注雌雄与不同年龄阶段的图例  常驻ui显示【样子：小/中/老  雌雄】
背景从默认的蓝色改成纯黑色
曲线图用鼠标拖动时的移速修改
初始8只老鼠是调用add【写死固定8只即可】
成年鼠颜色调整【黄色老鼠改成亮黄色】 或者自行上传文件+改路径
老鼠比例大小【需要一个说明文档 按照说明文档修改就行】
```
...
【intro暂时不修改】
```
引用 arXiv 确实是目前 AI 领域非常普遍的做法（因为领域发展太快）。但是，为了提升报告的 权威性、规范性和持久性，有比单纯引用 arXiv 链接更好的方式。
% 最高优先级：正式发表的会议/期刊 (Peer-reviewed)
% 如果文章已经被顶级会议（如 NeurIPS, ICLR, ICML, ACL, CVPR）或期刊录用，必须引用正式发表的版本，而不是 arXiv。这代表了同行评审的认可。

% 中等优先级：arXiv 预印本 (Preprint)
% 如果尚未发表，引用 arXiv 是标准的。但格式要规范，需包含 eprint、archivePrefix 和 primaryClass 字段，而不仅仅是 url。

% 基础优先级：代码仓库/软件 (Software/Misc)
% 对于纯工程项目（如 Dify, Coze），如果没有对应论文，使用 @misc 引用 GitHub 是对的。但建议添加 note 字段说明访问日期，或者使用 howpublished 字段。
```
...
okk【美术需求】食槽中放食物【场景搭建】
【后端需求】提供有顶房间坐标 前端按区域隐藏和显示房间内人数
...
要求模型打包部署 并把推理封装成后端服务
【不急 每周照常训练 已打包和形成一版文档】
...
- [ ] 
- [x] 
- [x] 下周一 画多版牛油果图标 见下载文件夹
- [x] 下周一 论文fig1配色重画 架构样子重画 见boardmix 7日内有效

- [ ] unity杂项需求。
【打包到web 尝试】
【校园要求平滑添加log 可显示时间okk 【debug】 需求是可大规模移动！ 自测2k很流畅 5k到10k就稳定低帧率】
下周一 校园move要debug出来 全体move【只是队列处理太慢】 考虑
